...
namespace ICP {
    ...
    public:
        Parameters() : f(NONE),
            p(0.1),
            max_icp(100),//最大ICP迭代次数，即ICP算法中内循环的最大迭代次数。
            max_outer(1),//最大外循环迭代次数，可能在整个ICP算法中有多个外循环。
            stop(1e-6),//ICP算法的停止标准，用于确定何时停止迭代。
            use_AA(false),//是否使用Anderson Acceleration（AA），一种用于加速迭代过程的优化技术。
            print_energy(false),//是否打印能量（error）的信息。
            print_output(false),//结果输出的路径。
            anderson_m(5),//Anderson Acceleration中的参数。
            beta_(1.0),//用于控制迭代过程的参数。
            error_overflow_threshold_(0.05),//误差溢出阈值。
            ...
            nu_begin_k(5),
            nu_end_k(1.0/(5.0*sqrt(5.0))),
            use_init(false),
            nu_alpha(1.0/2) {}
        // Parameters
        Function f;     /// robust function type 指定鲁棒函数（Robust Function）的类型
        // ICP::NONE：无鲁棒函数，即使用普通的最小二乘法进行配准。
        // ICP::WELSCH：Welsch鲁棒函数，一种基于Huber损失的鲁棒函数，对异常值具有较好的鲁棒性。
        ...
        bool use_AA;  /// whether using anderson acceleration 是否使用Anderson Acceleration（AA），一种用于加速迭代过程的优化技术。
        std::string out_path;
        ...
        MatrixXX init_trans;//初始变换矩阵。
        MatrixXX gt_trans;//地面真值变换矩阵。
        bool has_groundtruth;//是否提供地面真值信息。
        double convergence_energy;//收敛时的能量值。
        int convergence_iter;//收敛时的迭代次数。
        double convergence_gt_mse;//收敛时的地面真值均方误差。
        MatrixXX res_trans;//最终的变换矩阵。
        double nu_begin_k;//动态韦尔希（Welsch）参数的开始值。
        double nu_end_k;//动态韦尔希（Welsch）参数的结束值。
        bool use_init;//是否使用提供的初始变换矩阵。
        double nu_alpha;//动态韦尔希（Welsch）参数的调整因子。
    };
    /// Weight functions
    /// @param Residuals
    /// @param Parameter
    ...
    /// @param Residuals
    /// @param Parameter
    //Eigen::VectorXd& r：这是一个包含残差值的向量，函数将为每个残差计算相应的 Welsch 权重。
    //double p：这是 Welsch 函数的控制参数，它影响权重的衰减速度。通常，p 被设置为某个与数据特性相关的值。
